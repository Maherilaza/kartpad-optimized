...
for source_name in ['KartPadRuntimeOverlayHost.mm', 'KartPadMiiManager.mm']:
...
newlist=out/'KartPad.LinkFileList';newlist.write_text('\n'.join(str(out/pathlib.Path(p).name) if pathlib.Path(p).name in ['KartPadRuntimeOverlayHost.o','KartPadMiiManager.o'] else p for p in objects)+'\n')
...
info=plistlib.loads((app/'Info.plist').read_bytes());info.update(plistlib.loads((out/'asset-info.plist').read_bytes()));info['CFBundleShortVersionString']='0.4.23';info['CFBundleVersion']='46'
