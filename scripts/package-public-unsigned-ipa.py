...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="?",
        help="Output IPA path (defaults to artifacts/KartPad-v0.4.22-ios.1-unsigned.ipa)",
    )
    ...
    output = (
        args.output.resolve()
        if args.output
        else repo / "artifacts/KartPad-v0.4.22-ios.1-unsigned.ipa"
    )
    ...
    additional_entries = {
        ...
        "RELEASE_NOTES.md": repo / "docs/releases/v0.4.22-ios.1.md",
        "SOURCE_AND_REBUILD.md": repo / "docs/releases/v0.4.22-ios.1-source.md",
